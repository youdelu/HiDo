	...
	private static class FormatProcess {
		...
		public String perform() {
			...
			while (this.tokens.hasMoreTokens()) {
				...
				if ((this.afterByOrSetOrFromOrSelect)
					...
					closeParen();
				} else if (SQLFormat.BEGIN_CLAUSES
						.contains(this.lcToken)) {
					beginNewClause();
				} else if (SQLFormat.END_CLAUSES
						.contains(this.lcToken)) {
					endNewClause();
		...
		private static boolean isFunctionName(String tok) {
			...
			boolean isIdentifier = (Character.isJavaIdentifierStart(begin))
					|| ('"' == begin);
			return (isIdentifier)
					&& (!SQLFormat.LOGICAL.contains(tok))
					&& (!SQLFormat.END_CLAUSES.contains(tok))
					&& (!SQLFormat.QUANTIFIERS.contains(tok))
					&& (!SQLFormat.DML.contains(tok))
